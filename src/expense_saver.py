...
def expense_filter(
    expense: Expense, travel_thresholds: dict, group_thresholds: dict
) -> bool:
    try:
        ...
        if (
            expense.is_transport_expense() or expense.is_overnight_expense()
        ) and price_per_unit is not None:
            if (
                exp_type not in travel_thresholds
                or price_per_unit >= travel_thresholds[exp_type]
            ):
                return True
            return False
        ...
        group = expense.group()
        if (
            group not in group_thresholds
            or expense.amount_claimed >= group_thresholds[group]
        ):
            return True
